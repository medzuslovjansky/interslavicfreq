...
def _get_cache_path(dic_path: Path) -> Path:
    ...
    # Вариант 1: хранить кэш рядом с файлом (удобно для портативности, но требует прав записи)
    # return dic_path.with_suffix(".cache.pickle")
    
    # Вариант 2: хранить в системном кэше (чище)
...
def load_hunspell(dic_path: str | Path, aff_path: str | Path, *, 
    ...
    if use_cache and cache_path.exists():
        try:
            ...
            pass  # Если кэш повреждён или ошибка чтения, пересоздаём
    ...
    if use_cache:
        try:
            ...
            if not quiet:
                # Ошибка записи кэша не должна ломать работу (например, нет прав)
